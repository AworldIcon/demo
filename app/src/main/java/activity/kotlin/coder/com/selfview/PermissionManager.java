package activity.kotlin.coder.com.selfview;
...
public class PermissionManager {
    ...
    private void request(Object object, String[] permissions, int requestCode) {
        ...
        if (deniedPermissions.size() > 0) {
            ...
            if (mListener != null) {
                mListener.onGranted(permissions);
            }
        }
    }
    ...
    public void onPermissionResult(String[] permissions, int[] results) {
        ...
        List<String> grantPermissions = new ArrayList<String>();
        for (int i = 0; i < results.length; i++) {
            if (results[i] != PackageManager.PERMISSION_GRANTED) {//未授权
                deniedPermissions.add(permissions[i]);
            }else {
                grantPermissions.add(permissions[i]);//授权的
            }
        }
        if (deniedPermissions.size() > 0) {
            if (mListener != null) {
                mListener.onDenied(deniedPermissions.toArray(new String[deniedPermissions.size()]));
            }
        } else {
            if (mListener != null) {
                mListener.onGranted(grantPermissions.toArray(new String[grantPermissions.size()]));
            }
        }
    }
